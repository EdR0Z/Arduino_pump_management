...
const int seuilAutoPompe1 = 50;
const int seuilAutoPompe2 = 30;
...
void loop() {
  ...
  if (digitalRead(boutonModePin) == LOW) {
    modeManuel = false;                 // Mode automatique
    digitalWrite(ledSystemePin, HIGH);  // Éteindre la LED système en mode automatique
    delay(250);
  } else {
  ...
  if (currentTime - lastDistanceReadTime >= distanceReadInterval) {
    distance = sonar.ping_cm();  // Lire la distance en mode automatique aussi
  ...
  if (modeManuel) {
    ...
    if (currentTime - previousMillisSystem >= intervalSystem) {
      ...
      digitalWrite(ledSystemePin, !digitalRead(ledSystemePin));  // Inversion de l'état de la LED système pour le clignotement
    }
